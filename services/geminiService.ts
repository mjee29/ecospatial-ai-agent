...
7. 활성화된 레이어가 있다면 해당 리스크 유형 언급 (침수위험, 녹지현황 등)
8. 복합 리스크가 있다면 강조 (예: 침수위험+취약계층, 녹지부족+고령인구 등)
  ...
  try {
    ...
    const requestPromise = ai.models.generateContent({
      ...
      config: {
        ...
        [데이터 소스]
        - 경기도 기후플랫폼 WMS: 침수흔적지도, 극한호우 위험도, 녹지 현황
        ...
        2. 기후 리스크(침수)와 사회적 취약성(노인 인구, 고령인구, 대기질)을 동시에 언급하면 여러 레이어를 활성화하여 '중첩 분석(Overlap Analysis)'을 수행하세요.
        ...
        6. 'updateMapLayers' 도구를 호출한 후에는 지도에 어떤 데이터가 표시되었는지, 그리고 그 데이터가 어떤 의미를 갖는지 전문가 수준의 한국어로 설명하세요.
           ...
           - 복합 분석: 대기질이 나쁜 경우, 고령인구가 많은 지역의 취약성, 침수 위험 등 복합 리스크를 강조
        7. "지도 보여줘"와 같은 단순 요청에는 현재 경기도에서 가장 이슈가 되는 기후 레이어(예: 대기질, 녹지현황)를 제안하며 보여주세요.
