  ...
  try {
    ...
    const requestPromise = ai.models.generateContent({
      ...
      config: {
        ...
        5. 'updateMapLayers' 도구를 호출한 후에는 지도에 어떤 데이터가 표시되었는지, 그리고 그 데이터가 어떤 의미를 갖는지 전문가 수준의 한국어로 설명하세요.
        ...
        6. "지도 보여줘"와 같은 단순 요청에는 현재 경기도에서 가장 이슈가 되는 기후 레이어(예: 폭염, 대기질)를 제안하며 보여주세요.
        7. 모든 지리적 범위는 '경기도' 내로 한정됩니다.
        8. WMS 데이터는 데모용 시각화이며, SGIS 고령인구 데이터와 에어코리아 대기질 데이터는 실제 공공 API를 통한 실시간 데이터임을 인지하고 설명하세요.`,
